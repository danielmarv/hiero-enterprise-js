...
import { TokenRevokeKycTransaction, PrivateKey } from "@hiero-ledger/sdk";
...
vi.mock("@hiero-ledger/sdk", async (importOriginal) => {
    ...
    return {
        ...actual,
        TokenRevokeKycTransaction: vi.fn(function () {
            return mocks.tx;
        }),
    };
});

describe("TokenRevokeKycOperation (via TokenService)", () => {
    ...
    it("revokes KYC on a token relationship for an account", async () => {
        await service.revokeKycToken({
            tokenId: "0.0.500",
            accountId: "0.0.700",
        });

        const tx = vi.mocked(TokenRevokeKycTransaction).mock.results[0].value;
    ...
    it("applies base TransactionOptions and additionalSigners", async () => {
        ...
        await service.revokeKycToken({
            tokenId: "0.0.500",
            accountId: "0.0.700",
            transactionMemo: "revoke kyc memo",
        ...
        const tx = vi.mocked(TokenRevokeKycTransaction).mock.results[0].value;
        expect(tx.setTransactionMemo).toHaveBeenCalledWith("revoke kyc memo");
